...
function displaySources(messageElement, sourcesSection) {
    // Skip adding sources for very short responses (likely greetings)
    ...
    // Only add sources UI if we have sources from text or we'll try to fetch them
    if (sourceFilenames.length > 0 || hasSubstantiveContent(messageText)) {
...
// Debug function to help understand sources processing
function debugSourcesProcessing(messageElement, sourcesSection) {
    console.log("Processing sources section:", sourcesSection);
    
    // Create a debug info container (hidden in production)
    const debugInfo = document.createElement('div');
    debugInfo.className = 'debug-info hidden';
    debugInfo.style.color = '#666';
    debugInfo.style.fontSize = '0.8rem';
    debugInfo.style.borderTop = '1px dashed #ccc';
    debugInfo.style.marginTop = '8px';
    debugInfo.style.paddingTop = '8px';
    
    // Only show in development environments (you can modify this check)
    ...
    if (isDevEnvironment) {
        ...
        debugInfo.innerHTML = `
            <div>Raw sources: ${sourcesSection}</div>
            <div>User ID: ${userIdentifier}</div>
        `;
        
        // Add a button to manually fetch sources
        const fetchButton = document.createElement('button');
        fetchButton.textContent = "Manual fetch sources";
        fetchButton.className = "text-xs bg-gray-200 rounded px-2 py-1 mt-1";
        fetchButton.onclick = async () => {
            try {
                ...
                if (response.ok) {
                    const data = await response.json();
                    debugInfo.innerHTML += `<div>Sources from API: ${JSON.stringify(data.sources)}</div>`;
                } else {
                    debugInfo.innerHTML += `<div>Error fetching sources: ${response.status}</div>`;
                }
            } catch (error) {
                debugInfo.innerHTML += `<div>Exception: ${error.message}</div>`;
            }
        };
        
        debugInfo.appendChild(fetchButton);
        messageElement.appendChild(debugInfo);
    }
}

// Update the existing function to improve sources extraction
function processMessageText(content) {
    // Extract sources section if it exists using a more robust pattern
    let mainContent = content;
    let sourcesSection = null;
    
    // Try various patterns to extract sources
    const sourcesPatterns = [
        /\n\nSources:\s*(.*?)$/s,
        /\n\nsources:\s*(.*?)$/s,
        /\n\nSOURCES:\s*(.*?)$/s,
...
// Function to fetch and display source information with both page numbering systems
async function fetchAndAddSources(messageElement, sourcesSection) {
    try {
        console.log("Fetching sources for section:", sourcesSection);
        // Parse the simple list of sources
        const sourceFilenames = sourcesSection.split(',').map(s => s.trim()).filter(s => s);
        ...
        switch (selectedLanguage) {
            case 'en':
                ...
                break;
            case 'fr':
                buttonText = 'Voir les sources';
                break;
            case 'ar':
                buttonText = 'عرض المصادر';
                break;
            default:
                buttonText = 'View Sources';
        }
        
        sourcesButton.innerHTML = `<i data-feather="book-open" class="mr-1"></i> ${buttonText}`;
        messageElement.appendChild(sourcesButton);
        ...
        sourcesButton.addEventListener('click', function() {
            if (sourcesList.style.display === 'none') {
                ...
                switch (selectedLanguage) {
                    case 'en':
                        this.innerHTML = `<i data-feather="chevron-up" class="mr-1"></i> Hide Sources`;
                        break;
                    case 'fr':
                        this.innerHTML = `<i data-feather="chevron-up" class="mr-1"></i> Masquer les sources`;
                        break;
                    case 'ar':
                        this.innerHTML = `<i data-feather="chevron-up" class="mr-1"></i> إخفاء المصادر`;
                        break;
                    default:
                        this.innerHTML = `<i data-feather="chevron-up" class="mr-1"></i> Hide Sources`;
                }
                ...
                switch (selectedLanguage) {
                    case 'en':
                        this.innerHTML = `<i data-feather="book-open" class="mr-1"></i> View Sources`;
                        break;
                    case 'fr':
                        this.innerHTML = `<i data-feather="book-open" class="mr-1"></i> Voir les sources`;
                        break;
                    case 'ar':
                        this.innerHTML = `<i data-feather="book-open" class="mr-1"></i> عرض المصادر`;
                        break;
                    default:
                        this.innerHTML = `<i data-feather="book-open" class="mr-1"></i> View Sources`;
                }
        ...
        switch (selectedLanguage) {
            ...
            case 'fr':
                ...
                break;
            case 'ar':
                sourcesHeader.textContent = 'المصادر:';
                break;
            default:
                sourcesHeader.textContent = 'Sources:';
        }
        ...
        // Fetch detailed source information if available
        const response = await fetch('/get_source_details', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
        ...
        if (response.ok) {
            ...
            sources.forEach(source => {
                ...
                if (!sourceMap[filename]) {
                    sourceMap[filename] = [page];
                } else if (page !== null && !sourceMap[filename].includes(page)) {
                    sourceMap[filename].push(page);
                }
            });
            ...
            // First try to use the detailed sources from the JSON
            if (Object.keys(sourceMap).length > 0) {
                for (const [filename, pages] of Object.entries(sourceMap)) {
                    // Filter out null or undefined page numbers
                    const validPages = pages.filter(p => p !== null && p !== undefined);
                    ...
                    if (validPages.length > 0) {
                        ...
                        switch (selectedLanguage) {
                            case 'en':
                                pageLabel = validPages.length > 1 ? 'Pages' : 'Page';
                                break;
                            case 'fr':
                                pageLabel = validPages.length > 1 ? 'Pages' : 'Page';
                                break;
                            case 'ar':
                                pageLabel = validPages.length > 1 ? 'الصفحات' : 'الصفحة';
                                break;
                            default:
                                pageLabel = validPages.length > 1 ? 'Pages' : 'Page';
                        }
                        
                        const pageStrings = validPages.map(p => String(p));
                        pageString = `, ${pageLabel}: ${pageStrings.join(', ')}`;
                    }
                    
                    sourcesHTML += `<div class="source-item">
                        <a href="/download_pdf?filename=${encodeURIComponent(filename)}" 
                           class="source-download-link" download target="_blank">
                           <i data-feather="file-text"></i>
                           ${filename}${pageString}
                        </a>
                    </div>`;
            ...
            // Fallback to the plain list if no detailed information
            else if (sourceFilenames.length > 0) {
                sourceFilenames.forEach(filename => {
                    sourcesHTML += `<div class="source-item">
                        <a href="/download_pdf?filename=${encodeURIComponent(filename)}" 
                           class="source-download-link" download target="_blank">
            ...
            // No sources
                ...
                // Set no sources message based on language
                let noSourcesMessage = '';
                switch (selectedLanguage) {
                    case 'en':
                        noSourcesMessage = 'No downloadable sources available.';
                        break;
                    case 'fr':
                        noSourcesMessage = 'Aucune source téléchargeable disponible.';
                        break;
                ...
                sourcesHTML = `<div class="source-item">
                    <span class="no-sources-available">${noSourcesMessage}</span>
                </div>`;
            }
            ...
            // Fallback to basic source display
            let sourcesHTML = '';
        ...
        // Fallback to a simple sources button
        const sourcesButton = document.createElement('button');
        sourcesButton.className = 'sources-button mt-3 text-blue-600 hover:text-blue-800 flex items-center text-sm font-medium';
        ...
        switch (selectedLanguage) {
            case 'en':
                buttonText = 'Sources unavailable';
                break;
            case 'fr':
                buttonText = 'Sources indisponibles';
                break;
            case 'ar':
                buttonText = 'المصادر غير متوفرة';
                break;
            default:
                buttonText = 'Sources unavailable';
        }
        ...
        sourcesButton.style.opacity = '0.6';
        messageElement.appendChild(sourcesButton);
